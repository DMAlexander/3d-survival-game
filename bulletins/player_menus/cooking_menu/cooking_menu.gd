...
func _ready() -> void:
	...
	starting_cooking_slot.mouse_extied.connect(hide_item_info)
	final_cooking_slot.mouse_entered.connect(show_item_info.bind(final_cookings_slot))
...
func uncooked_item_add() -> void:
	cooked_button.disabled = false
...
func start_cooking() -> void:
	...
	cook_button.disabled = true
	cooking_progress_bar.value = time_cooked / cooking_recipie.cooking_time
...
func close() -> void:
	EventSystem.BUL_destory_bulletin.emit(BulletinConfig.Keys.CookingMenu)
