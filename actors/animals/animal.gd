...
@onready var idel_timer: Timer = $Timers/IdleTimer
@onready var wander_timer: Timer = $timers/WanderTimer
@onready var disappear_after_death_timer: Timer = $Timer/DisappearAfterDeathTimer
...
func idle_loop() -> void:
	apply_gravity()
	if is_aggressive and can see_layer():
		set_state(States.Chase)
...
func pick_away_from_player_velocty() -> void:
	if not player:
		set_state(States.Idle)
		return
